...
const MOCK_ID_LIST = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n']
const makeSocketId = () => {
    const id = MOCK_ID_LIST[Math.floor(Math.random() * MOCK_ID_LIST.length)];
    if (id in sockets) {
        return makeSocketId()
    }
    return id;
    ...
    ws.on('message', message => {
        ...
        switch (msg.type) {
            ...
            case 'CREATE_ANSWER':
                ...
                break;
            case 'CANDIDATE':
                sockets[msg.data.toId].send(toStringMessage('CANDIDATE', {fromId: id, candidate: msg.data.candidate}))
            default:
                break;
        }
